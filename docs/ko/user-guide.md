...
Candidate card에서 Research next, Prototype next, Park, Reject를 누르면 browser-local Experiment Brief가 만들어집니다. 저장 키는 `research-to-project-lab.experimentBriefs.v1`이며 서버 동기화가 아닙니다. Prototype next는 smallest test와 success criteria가 필요하고, Park/Reject는 이유를 남겨야 auditable decision이 됩니다.
