...
function actionButtons(candidate) {
  ...
  return actions
    .map(
      (action) => `
        <button class="button button--${action.kind}" type="button" aria-label="${action.label} ${candidate.title}">
...
main().catch((error) => {
  document.querySelector("#candidate-list").innerHTML = `
    <p class="error" role="alert">Unable to load candidate data: ${error.message}</p>
  `;
});
